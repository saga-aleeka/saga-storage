...
export function UserManagement({ 
  ...
  // Edit user form state (separate from newUser to avoid collisions)
  ...
  const loadRoles = () => {
    try {
      ...
      if (saved) {
        const savedRoles = JSON.parse(saved) as UserRole[];
        // Merge saved roles with defaults to guarantee presence of defaults
        const mergedDefaults = DEFAULT_ROLES.map(def => savedRoles.find(r => r.id === def.id) || def);
        // Include any custom roles that aren't part of defaults
        const custom = savedRoles.filter(r => !DEFAULT_ROLES.some(d => d.id === r.id));
        setRoles([...mergedDefaults, ...custom]);
      } else {
  ...
  const createUser = () => {
    ...
    const duplicate = users.find(u => 
      u.username === newUser.username || u.email.toLowerCase() === newUser.email.toLowerCase()
    );
    if (duplicate) {
  ...
  const updateUser = (userId: string, updates: Partial<User>) => {
    ...
    // If updating current user, update context + cache
    if (currentUser?.id === userId && onUserChange) {
      const updatedUser = updatedUsers.find(u => u.id === userId);
      const userRole = updatedUser ? roles.find(r => r.id === updatedUser.roleId) : undefined;
      if (updatedUser && userRole) {
        const merged: CurrentUser = { ...updatedUser, role: userRole };
        onUserChange(merged);
        try {
  ...
  const canManageUsers = () => {
    if (onPermissionCheck) return onPermissionCheck('manageUsers');
    return currentUser?.role.permissions.manageUsers || false;
  };

  const hasPermission = (permission: keyof UserRole['permissions']) => {
    if (onPermissionCheck) return onPermissionCheck(permission);
  ...
  return (
    <div className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Select
                    value={editUserForm.roleId || selectedUser.roleId}
                    onValueChange={(val) => setEditUserForm(prev => ({ ...prev, roleId: val }))}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
      ...
      {/* Role Management Modal (read-only summary) */}

...
export function StorageDashboard() {
  ...
  const getContainerIcon = (type: StorageContainer['type']) => {
    switch (type) {
  ...
  const getOccupancyVariant = (occupied: number, capacity: number) => {
  ...
  const getProgressBarClass = (occupied: number, capacity: number) => {
    const pct = (occupied / capacity) * 100;
  ...
  const renderContainerView = (container: StorageContainer) => {
    switch (container.type) {
      ...
      default:
        return <div className="text-sm text-muted-foreground p-4">Unknown container type</div>;
    }
  };
    ...
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {mockContainers.map((container) => {
          const pct = (container.occupied / container.capacity) * 100;
          const progressClass = getProgressBarClass(container.occupied, container.capacity);
          ...
          return (
            <Card
              ...
              role="button"
              tabIndex={0}
              aria-label={`${container.name}, ${container.occupied} of ${container.capacity} occupied, located in ${container.location}`}
              className="p-4 cursor-pointer hover:shadow-md transition-shadow outline-none focus:ring-2 focus:ring-primary"
            ...
            >
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <Badge variant={getOccupancyVariant(container.occupied, container.capacity)}>

...
function Sidebar({
  ...
  return (
    <div
      className="group peer hidden text-slate-900 dark:text-slate-100 md:block"
      data-state={state}
      ...
      <div
        ...
        className={cn(
          ...
          variant === "floating" || variant === "inset"
            ...
            : "group-data-[collapsible=icon]:w-[var(--sidebar-width-icon)] group-data-[side=left]:border-r group-data-[side=right]:border-l border-slate-200 dark:border-slate-800",
          className
        )}
        {...props}
      >
        <div
          ...
          className={cn(
            ...
            variant === "floating" &&
              "rounded-lg border shadow-sm border-slate-200 dark:border-slate-800",
            "bg-white text-slate-900 dark:bg-slate-900 dark:text-slate-100"
          )}
        >
          {children}
  ...
  return (
    <button
      ...
      className={cn(
        ...
        "hover:after:bg-slate-200 dark:hover:after:bg-slate-700",
        "transition-all ease-linear",
        "group-data-[side=left]:-right-4 group-data-[side=right]:left-0",
        "[[data-side=left][data-state=collapsed]_&]:cursor-e-resize",
        "[[data-side=right][data-state=collapsed]_&]:cursor-w-resize",
        "hover:group-data-[collapsible=offcanvas]:bg-white/60 dark:hover:group-data-[collapsible=offcanvas]:bg-slate-800/30",
...
function SidebarInput({ className, ...props }: React.ComponentProps<typeof Input>) {
  return (
    <Input
      ...
      data-sidebar="input"
      className={cn("h-8 w-full bg-white dark:bg-slate-900 shadow-none", className)}
      {...props}
    />
  );
}
...
function SidebarGroupLabel({
  ...
  return (
    <Comp
      ...
      className={cn(
        "flex h-8 shrink-0 items-center gap-2 rounded-md px-2 text-xs font-medium text-slate-500 dark:text-slate-400",
...
const sidebarMenuButtonVariants = cva(
  [
    ...
    "active:bg-slate-100 active:text-slate-900 dark:active:bg-slate-800/60 dark:active:text-slate-100",
    "disabled:pointer-events-none disabled:opacity-50",
    "group-has-[data-sidebar=menu-action]/menu-item:pr-8",
    "aria-disabled:pointer-events-none aria-disabled:opacity-50",
    "data-[active=true]:bg-slate-100 data-[active=true]:font-medium dark:data-[active=true]:bg-slate-800/60",
    "data-[state=open]:hover:bg-slate-100 dark:data-[state=open]:hover:bg-slate-800/60",
  ...
  ].join(" "),
  {
    variants: {
      variant: {
        default: "",
        outline:
          "bg-white dark:bg-slate-900 ring-1 ring-slate-200 dark:ring-slate-700 hover:ring-blue-300/60 dark:hover:ring-blue-400/40",
      },
      size: {
        default: "h-8 text-sm",
        sm: "h-7 text-xs",
...
function SidebarMenuSkeleton({
  ...
  return (
    <div data-slot="sidebar-menu-skeleton" data-sidebar="menu-skeleton" className={cn("flex h-8 items-center gap-2 rounded-md px-2", className)} {...props}>
      ...
      <Skeleton className="h-4 flex-1 max-w-[var(--skeleton-width)]" data-sidebar="menu-skeleton-text" style={{ "--skeleton-width": width } as React.CSSProperties} />
    </div>
  );
}
...
function SidebarMenuSub({ className, ...props }: React.ComponentProps<"ul">) {
  return (
    <ul
      ...
      className={cn(
        "mx-3.5 flex min-w-0 translate-x-px flex-col gap-1 border-l px-2.5 py-0.5 border-slate-200 dark:border-slate-800",
        "group-data-[collapsible=icon]:hidden",
        className
      )}
      {...props}
  ...
  return (
    <Comp
      ...
      className={cn(
        ...
        "hover:bg-slate-100 active:bg-slate-100 dark:hover:bg-slate-800/60 dark:active:bg-slate-800/60",
        "focus-visible:ring-2 focus-visible:ring-blue-500/50",
        "disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50",
        "[&>span:last-child]:truncate [&>svg]:h-4 [&>svg]:w-4 [&>svg]:shrink-0",
        "data-[active=true]:bg-slate-100 data-[active=true]:text-slate-900 dark:data-[active=true]:bg-slate-800/60 dark:data-[active=true]:text-slate-100",

...
import type { ContainerType, SampleType, PlasmaContainer } from "./PlasmaContainerList";
...
// Match admin logic: auto-detect container type from sample type
const detectContainerTypeFromSampleType = (
...
export function EditContainerDialog({
  ...
  const [showContainerTypeWarning, setShowContainerTypeWarning] =
    useState(false);
  ...
  const handleSampleTypeChange = (value: SampleType) => {
    const recommended = detectContainerTypeFromSampleType(value);
    const currentType = formData.containerType;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Safety: ensure onUpdateContainer is callable; fallback to localStorage if not
    if (typeof onUpdateContainer !== "function") {
      try {
        ...
        const saved = localStorage.getItem("plasma-containers");
        if (saved) {
          const list = JSON.parse(saved);
          const next = list.map((c: PlasmaContainer) =>
            c.id === updatedContainer.id ? updatedContainer : c,
          );
          localStorage.setItem("plasma-containers", JSON.stringify(next));
          alert(
            "Changes saved (fallback). Please refresh the page to see updates.",
          );
          onOpenChange(false);
          return;
        }
      ...
      alert("Error: Unable to save changes. Please try again.");
      return;
    }

    // Extra confirmation if changing container type with existing samples
    const isTypeChanging = container.containerType !== formData.containerType;
    const hasSamples = container.occupiedSlots > 0;

    if (isTypeChanging && hasSamples) {
      const confirmChange = window.confirm(
        `Warning: You are changing the container type from ${getContainerTypeLabel(
          container.containerType,
        )} to ${getContainerTypeLabel(
          formData.containerType,
        )}. This container has ${container.occupiedSlots} samples.\n\nChanging the container type may affect sample positions. Are you sure you want to continue?`,
      );
  ...
  const selectedDimensions = getGridDimensions(
    formData.containerType,
    formData.sampleType,
  );
  const recommendedContainerType = detectContainerTypeFromSampleType(
    formData.sampleType,
  );
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="w-[400px] sm:w-[500px] overflow-y-auto">
        <SheetHeader>
          ...
          <SheetDescription>
            Update the container settings. Be careful when changing container
            type if samples are already stored.
          </SheetDescription>
        </SheetHeader>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-4">
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between p-4 border rounded-lg">
                  <div className="space-y-1">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Mark this container as a training container (highlighted
                      in orange)
                    </p>
                  </div>
                  <Switch
                    ...
                    onCheckedChange={(checked) =>
                      setFormData({ ...formData, isTraining: checked })
                    }
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v) => handleSampleTypeChange(v as SampleType)}
                >
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  ...
                  {getContainerTypeLabel(recommendedContainerType)}
                </p>
              </div>
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    handleContainerTypeChange(v as ContainerType)
                  }
                >
              ...
              <div>
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="-80°C">
                      -80°C (Ultra-low freezer)
                    </SelectItem>
                    <SelectItem value="-20°C">
            ...
            {showContainerTypeWarning && (
              <Alert variant="destructive">
                ...
                <AlertDescription>
                  <strong>Warning:</strong> This container has{" "}
                  {container.occupiedSlots} samples. Changing the container type
                  may affect sample positions and grid layout.
                </AlertDescription>
              </Alert>
            )}
            ...
            >
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  <span>Grid Size:</span>
                  <span>
                    {selectedDimensions.rows}×{selectedDimensions.cols}
                  </span>
                </div>
                <div className="flex justify-between">
              ...
              <div className="mt-4">
                ...
                >
                  {Array.from(
                    { length: Math.min(selectedDimensions.rows, 5) },
                    (_, row) => (
                      <div key={row} className="flex gap-1">
                        {Array.from(
                          { length: Math.min(selectedDimensions.cols, 5) },
                          (_, col) => (
                            <div
                              key={col}
